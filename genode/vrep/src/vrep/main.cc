...
void Component::construct(Genode::Env &)
{
	...
	try {
		Genode::Xml_node vrep = Genode::config()->xml_node().sub_node("vrep");
		vrep.attribute("ip-address").value(ip_addr, sizeof(ip_addr));
		vrep.attribute("port").value(&port);
	} catch(Genode::Xml_node::Nonexistent_sub_node) {
		Genode::error("vrep configuration missing");
	} catch(Genode::Xml_node::Nonexistent_attribute) {
		Genode::error("ip-address or port missing in vrep configuration");
		return;
	}

	int clientID;
	do {
		clientID = simxStart((simxChar*)ip_addr, port, true, true, 2000, 5);
	} while(clientID != 0);

	Genode::log("connected to vrep");
